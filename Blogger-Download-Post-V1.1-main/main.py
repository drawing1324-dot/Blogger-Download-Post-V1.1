...
def process_blog(
    ...
    try:
        ...
        if workflow.is_enabled(
            ...
            targets = source_adapter.build_search_targets(blog["type"], title)
            source_result = None
            for target in targets:
                candidate = search.search(title, target["source"])
                if candidate.get("found_url"):
                    source_result = candidate
                    break
            if source_result is None:
                logger.warning("No download source found", {"title": title})
                return
        else:
            source_result = {"found_url": None}



        # Image

        if workflow.is_enabled("find_images"):
            image_result = image.find_image(
                title,
                (source_result or {}).get("found_url")
            )
        else:
